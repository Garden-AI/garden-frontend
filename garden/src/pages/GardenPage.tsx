...
const GardenPage = ({ bread }: { bread: any }) => {
  ...
  const getDatasetListFromResult = (result: any): Array<Object> => {
    ...
    for (let entrypoint of entrypoints) {
      doiToDataset[entrypoint.doi] = entrypoint.datasets[0];
    }
  ...
  const NoDatasets = () => <p className="text-center pt-8 pb-16 text-base sm:text-xl col-span-2">No datasets available for this garden</p>;
  ...
  return (
    <div className="font-display">
      <div autoFocus className="h-full w-full flex flex-col gap-10 px-6 sm:px-16 md:px-36 py-12 sm:py-20 font-display">
        ...
        <div className="flex gap-4 sm:gap-8">
          ...
          <div className="flex gap-3 items-center">
            ...
            <Modal show={show} close={closeModal} copy={copy} doi={result[0]?.entries[0].content.doi} showTooltip={showTooltip} />
          </div>
        </div>
        ...
        <div className="border-0 rounded-lg bg-gray-100 flex flex-col gap-5 p-4 text-sm text-gray-700">
          <div>
            ...
            <p>
              {result[0]?.entries[0].content.authors.map((author: any, index: number) => (
                <span>{author}</span>
              ))}
            </p>
          </div>
          <div>
            ...
            <a href={`https://doi.org/${result[0]?.entries[0].content.doi}`} target="_blank" rel="noopener noreferrer" className="text-green underline">
            ...
            {active === 'Datasets' && (
              <div>
                <div className="mx-6 sm:mx-16 text-base sm:text-xl pb-4">
                  Below are the datasets that are used in this garden. Clicking on the URL will take you to where they are hosted, and allow you to learn more about them and how to view them.
                </div>
                <div>
                  <div className="grid grid-cols-1 gap-2 md:grid-cols-2 sm:gap-12 lg:px-24 pb-4">
                    {datasets.length > 0 ? datasets.map((dataset) => <DatasetBoxEntrypoint dataset={dataset} showFoundry={foundry} />) : <NoDatasets />}
                  </div>
                </div>
                {showFoundry === true ? (
                  <div>
      ...
      <div className="relative flex items-center pb-12">
        <button className="w-16 h-16 ml-4 sm:ml-12 mr-2 sm:mr-6 bg-gray-100" onClick={leftScroll}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="object-fit">
            <path strokeLinecap="round" strokeLinejoin="round" d="M15.75 19.5L8.25 12l7.5-7.5" />
          </svg>
        </button>
        <div id="related" className="w-full h-full overflow-x-scroll scroll-smooth whitespace-nowrap inline-flex gap-4">
          {relatedResults.map((related) => (
            <RelatedGardenBox related={related} />
          ))}
        </div>

        <button className="w-16 h-16 ml-2 mr-4 sm:ml-6 sm:mr-12 bg-gray-100" onClick={rightScroll}>
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="object-fit">
