...
const MetadataEditing = () => {
  ...
  return (
    <div className="mx-auto max-w-7xl px-8 py-4 font-display md:py-16">
      ...
      />
      <h1 className="mb-4 text-2xl sm:text-3xl">Edit '{currGarden?.title}'</h1>
      <div className="flex flex-col gap-5 rounded-lg border-0 bg-gray-100 p-4 text-sm text-gray-700">
        <div className="space-y-2">
          ...
          <input
            ...
            placeholder="Title"
            className="w-full rounded border border-gray-200 px-2 py-1 focus:border-2 focus:border-green focus:outline-none focus:ring-0"
          />
        </div>
        <div className="space-y-2">
          ...
          <MultipleSelector
            ...
            creatable
            value={metadata.contributors?.map((contributor) => ({
              label: contributor,
              value: contributor,
            }))}
            onChange={(newValue) =>
