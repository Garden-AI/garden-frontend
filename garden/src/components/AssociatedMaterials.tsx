...
import { Entrypoint, ModalFunction } from "@/api/types";
      ...
      <AccordionItem value="materials">
        <AccordionTrigger className="px-4 py-2 transition-colors duration-200 hover:bg-gray-100 data-[state=open]:bg-gray-100">
          <div className="flex items-center gap-x-2 p-2">
            ...
            <span className="font-medium text-gray-700">Associated Materials</span>
          </div>
        </AccordionTrigger>
        <AccordionContent className="grid grid-cols-1 gap-4 py-6 lg:grid-cols-2">
          <>
            ...
            {resource.repositories?.map(({ repo_name, contributors, url }, index) => (
              <div
                key={index}
              ...
              >
                ...
                <div className="flex justify-between">
                  <div className="flex items-center gap-2">
                    <p>Copy Link</p>
                    <CopyButton content={url} hint="Copy Repository URL" className="" />
                  </div>
                  <Button variant={"secondary"} className="mb-2 text-sm text-gray-600" asChild>
            ...
            {resource.papers?.length === 0 && resource.repositories?.length === 0 && (
              <div className="rounded-md bg-white p-6">
                <p className="text-base text-gray-800 ">No associated materials available</p>
              </div>
            )}
          </>
        </AccordionContent>
