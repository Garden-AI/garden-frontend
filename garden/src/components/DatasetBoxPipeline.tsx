...
const DatasetBoxPipeline = (props: { dataset: any; showFoundry: Function }) => {
  ...
  useEffect(() => {
    if (props.dataset.url.contains("foundry")) {
      props.showFoundry();
    }
  },[props])
  ...
  return (
    <div className="flex flex-col gap-2 border border-gray rounded-lg px-4">
      ...
      <a target="blank" className="break-words" href={props.dataset.url}>
        {props.dataset.url}
      </a>
      <div className="pb-4">
        {props.dataset.doi ? <p className="p-1">DOI: {props.dataset.doi}</p> : <></>}
        {props.dataset.type ? (
          <p className="p-1">
            File Type:{" "}
            {props.dataset.type}
